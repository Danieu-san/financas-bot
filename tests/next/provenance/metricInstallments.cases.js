...
function fixture(change = () => {}) {
    ...
    const links = ['first', 'second', 'third'].map(source => ({ id: source, source, target: 'plan', field: 'installment_plan', type: 'ref' }));
    ...
    const ctx = period => createInstrumentedAccess({ bindings: [{ alias: 'ctx', role: 'context',
        value: { subject: { kind: 'installment_plan', ref_id: 'plan' }, period, time_basis: 'installment_competence' },
    ...
    return { operands: period => ({ context: ctx(period), plan: nodes.handle('plan'), events: set.handle }), observations };
}
