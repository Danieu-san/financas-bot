...
test('instalador usa S4U limitado e executa somente cópia protegida em ProgramData', () => {
    ...
    assert.match(installer, /Set-BridgeAcl \$bridgeRoot 'ReadAndExecute' 'ReadAndExecute'/);
    assert.match(installer, /Set-BridgeAcl \$binPath 'ReadAndExecute' 'ReadAndExecute'/);
    assert.match(installer, /Set-BridgeAcl \$inboxPath 'Modify' 'Modify'/);
    assert.match(installer, /Set-BridgeAcl \$statePath 'Modify' 'ReadAndExecute'/);
    ...
    assert.match(installer, /Lock da ponte pertence ao processo vivo/);
    assert.equal((installer.match(/Remove-Item -LiteralPath \$lockPath -Force/g) || []).length, 1);
    assert.match(installer, /'Install' \{[\s\S]*?Assert-BridgeLifecycleSafe/);
    assert.match(installer, /'Remove' \{[\s\S]*?Assert-BridgeLifecycleSafe/);
