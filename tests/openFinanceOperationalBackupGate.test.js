...
test('9F operational gate creates a retained backup and destroys only the isolated restore', async () => {
    ...
    const files = Object.fromEntries(['staging', 'baseline', 'outbox', 'journal', 'secret', 'mappings', 'backups']
        .map(name => [name, path.join(root, name.includes('backup') ? name : `${name}.${['staging','baseline','outbox','journal'].includes(name) ? 'sqlite' : name === 'secret' ? 'txt' : 'json'}`)]));
