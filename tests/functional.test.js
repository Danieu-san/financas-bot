...
functionalTest('functional: consent, onboarding, settings and dashboard', { concurrency: false }, async () => {
    await withFunctionalState(FUNCTIONAL_SENDERS.surface, async ({ activateAndOnboard, send }) => {
        ...
        assert.match(last(await send('dashboard')), /\/dashboard\/v2#token=/, 'Dashboard deve abrir o v2 por padrao e manter token no fragmento');
        assert.match(last(await send('dashboard v1')), /\/dashboard#token=/, 'Dashboard v1 deve permanecer disponivel como rollback explicito');
