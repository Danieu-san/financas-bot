    ...
    if (report.readyForManualReview) {
        ...
        return {
            ...
            message: [
                ...
                `Precisão dos candidatos a auto-save: ${(Number(report.autoSaveCandidatePrecision || 0) * 100).toFixed(2)}%`,
                `Casos ambíguos gravados automaticamente: ${Number(report.ambiguousAutoSaveViolations || 0)}`,
                `Chamadas Gemini adicionais: ${Number(report.additionalGeminiCalls || 0)}`,
                `Latência p95 da avaliação local: ${Number(report.evaluationLatencyP95Ms || 0).toFixed(2)} ms`,
