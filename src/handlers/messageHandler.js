...
function detectSecuritySensitiveRequest(messageBody) {
    ...
    const internalIdentifierPatterns = [
        ...
        /\b(?:sheet|spreadsheet|tenant|workspace|user|usuario|cliente)[_\s-]*id\b/,
...
function sanitizeAnalyticalParametersForContext(parameters = {}) {
    const safe = {};
    ['mes', 'ano', 'categoria', 'cartao', 'origem', 'scope', 'member', 'status', 'source', 'metric'].forEach((key) => {
...
function storeAnalyticalContext(senderId, intentClassification = {}, meta = {}) {
    ...
    userStateManager.setState(key, {
        ...
        metric: sanitizeAnalyticalMetric(meta.metric || intentClassification.metric)
    }, ttlSeconds);
...
function deriveFollowUpAnalyticalQueryPlan(text, context) {
    ...
    const isDashboardContext = /dashboard/.test(normalizeText(context.intent));
    if (isDashboardContext) {
        const explicitMetric = /\b(disponivel|caixinha|reserva)\b/.test(text)
            ? 'available'
            : /\b(categoria|categorias)\b/.test(text)
                ? 'categories'
                : /\b(orcamento|ritmo)\b/.test(text)
                    ? 'budget'
                    : /\b(recente|ultimo|lancamento|lancamentos)\b/.test(text)
                        ? 'recentTransactions'
                        : '';
        const previousMetric = normalizeText(explicitMetric || context.metric || previousParams.metric || '');
        const metric = previousMetric.includes('available') || previousMetric.includes('disponivel')
            ? 'available'
            : previousMetric.includes('categor')
                ? 'categories'
                : previousMetric.includes('budget') || previousMetric.includes('orcamento')
                    ? 'budget'
                    : previousMetric.includes('recent') || previousMetric.includes('lancamento')
                        ? 'recentTransactions'
                        : 'balance';
        return {
            metric: context.metric || 'dashboard_followup',
            intent: 'dashboard_explicacao',
            parameters: { ...inheritedParams, metric, timeBasis: previousParams.timeBasis || 'transaction_date' }
        };
    }
...
function inferAnalyticalQueryPlan(userQuestion, previousContext = null) {
    ...
    const explicitDashboardMonthCount = (text.match(
    ...
    if (hasDashboardSignal || hasAvailabilitySignal) {
        ...
        if (hasDashboardSignal && /\bdisponivel\b/.test(text)) {
...
function openFinanceStateData(proposalRef, batch = null) {
    return {
        proposalRef,
        ...(batch ? { batch } : {})
    };
...
async function advanceQueuedOpenFinanceSaveReview({
    ...
    if (!batch?.queuedProposalRefs?.length) return null;
    const reviewCatalog = await buildOpenFinanceSaveProposalReviewCatalog({ userId });
    return advanceOpenFinanceSaveProposalBatch({
        batch,
...
async function processMessage(msg) {
    ...
    if (currentState?.action === 'awaiting_open_finance_save_selection') {
        let batchReply;
        try {
            batchReply = handleOpenFinanceSaveProposalBatchReply({
                messageBody,
                actorWhatsappId: senderId,
                proposals: currentState.data?.proposals || []
            });
        } catch (error) {
            ...
            try {
                ...
                logger.warn(`[open-finance] save_batch_catalog_unavailable sender=${logger.redactIdentifier(senderId)} ${logger.safeError(catalogError)}`);
                await sendPlainMessage(
                    msg,
                    'Não consegui abrir os dados de conferência agora. A lista continua pendente e nada foi salvo.'
                );
                return;
            }
            batchReply = handleOpenFinanceSaveProposalBatchReply({
    ...
    if (currentState?.action === 'awaiting_open_finance_save_batch_continue') {
        try {
            const advanced = await advanceQueuedOpenFinanceSaveReview({
                batch: currentState.data?.batch,
                senderId,
                userId
            });
            if (!advanced) {
            ...
            await sendPlainMessage(msg, advanced.reply);
        } catch (error) {
            logger.warn(`[open-finance] save_batch_resume_unavailable sender=${logger.redactIdentifier(senderId)} ${logger.safeError(error)}`);
            await sendPlainMessage(
                msg,
