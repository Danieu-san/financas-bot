...
const RESERVE_DECISIONS = new Set(['reserve_application', 'reserve_redemption']);
const RESERVE_REVIEW_STATUSES = new Set([
...
function requireSecret(secret) {
    ...
    if (value.length < 32) throw new Error('open_finance_reserve_proposal_secret_required');
    return value;
}
...
function reserveDirection(decision, amountCents) {
    if (decision === 'reserve_application' && Number(amountCents) < 0) return 'application';
    if (decision === 'reserve_redemption' && Number(amountCents) > 0) return 'redemption';
    throw new Error('open_finance_reserve_proposal_direction_changed');
}

function buildReviewedReserveSaveProposal({
    review,
    currentSource,
    ...
    if (!review || review.review_state !== 'decided' ||
        !RESERVE_DECISIONS.has(review.decision) || review.review_kind !== 'reserve' ||
        !RESERVE_REVIEW_STATUSES.has(review.review_status) ||
        !/^[a-f0-9]{32}$/.test(String(review.review_ref || '')) ||
        !/^[a-f0-9]{32}$/.test(String(review.observation_ref || ''))) {
        throw new Error('open_finance_reserve_proposal_review_not_approved');
    }
    if (!currentSource ||
        ...
        throw new Error('open_finance_reserve_proposal_source_changed');
    }
    const durableProviderOperation = String(review.provider_operation_type || '')
        .trim().toUpperCase();
    if (durableProviderOperation && durableProviderOperation !==
        String(currentSource.transaction.operation_type || '').trim().toUpperCase()) {
        throw new Error('open_finance_reserve_proposal_source_changed');
    }
    if (!reconciliationDecision || reconciliationDecision.status !== 'new' ||
        !/^[a-f0-9]{32}$/.test(String(reconciliationDecision.transaction_ref || ''))) {
        throw new Error('open_finance_reserve_proposal_not_new');
    }
    ...
    if (!/^[a-z0-9_-]{2,48}$/.test(alias) || !['daniel', 'thais'].includes(principal)) {
        throw new Error('open_finance_reserve_proposal_scope_invalid');
    }
    const direction = reserveDirection(review.decision, currentSource.transaction.amount_cents);
    const proposalRef = hmac(
        safeSecret,
        `reviewed-reserve-save-proposal:${review.review_ref}:${review.observation_ref}`
    );
    return {
        ...
        operation_key: hmac(
            safeSecret,
            `open-finance-reserve-write:${review.review_ref}:${review.observation_ref}`,
            48
        ),
...
async function prepareReviewedReserveSaveProposal({
    reviewCode,
    actorWhatsappId,
    userId,
    env = process.env,
    ...
    if (review.review_state !== 'decided' || !RESERVE_DECISIONS.has(review.decision)) {
        throw new Error('open_finance_reserve_proposal_review_not_approved');
    }
    ...
    if (matches.length !== 1) {
        throw new Error('open_finance_reserve_proposal_mapping_unavailable');
    }
    ...
    if (!item) throw new Error('open_finance_reserve_proposal_source_unavailable');
    ...
    if (!internalSource.available) {
        throw new Error('open_finance_reserve_proposal_internal_source_unavailable');
    }
    ...
    const proposal = buildReviewedReserveSaveProposal({
        review,
        currentSource: {
            alias,
            alias_ref: review.alias_ref,
    ...
    try {
        ...
        if (confirmation.state !== 'ready') {
            throw new Error('open_finance_reserve_proposal_confirmation_not_ready');
        }
    } finally {
        preview.close();
        journal.close();
    ...
    return {
        ...
        reply: [
            proposal.reserve_direction === 'application'
                ? 'Aplica\u00e7\u00e3o em reserva confirmada. Nada foi salvo ainda.'
                : 'Resgate de reserva confirmado. Nada foi salvo ainda.',
            `Valor: ${(Math.abs(Number(proposal.source.amount_cents)) / 100).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`,
            `Descri\u00e7\u00e3o: ${String(proposal.source.description || '').slice(0, 120)}`,
            '',
            'Quer continuar para escolher a conta e a reserva e conferir o salvamento?',
...
module.exports = {
    ...
    __test__: { sourceFingerprint, reserveDirection }
};
