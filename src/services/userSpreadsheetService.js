...
function buildInvoiceSummaryRows({ dataStartRow = 2 } = {}) {
    return [[
        '=IFERROR(LET(q;DROP(QUERY(HSTACK(ARRAYFORMULA(IF(TRIM(\'Lançamentos Cartão\'!G2:G)<>"";"id:"&TRIM(\'Lançamentos Cartão\'!G2:G);IF(TRIM(\'Lançamentos Cartão\'!H2:H)<>"";"legacy:"&TRIM(\'Lançamentos Cartão\'!H2:H);"")));\'Lançamentos Cartão\'!F2:F;\'Lançamentos Cartão\'!D2:D;\'Lançamentos Cartão\'!A2:A;\'Lançamentos Cartão\'!J2:J);"select Col1, Col2, sum(Col3), count(Col3), min(Col4), max(Col4) where Col5 is not null and Col1 is not null group by Col1, Col2 label Col1 \'\', Col2 \'\', sum(Col3) \'\', count(Col3) \'\', min(Col4) \'\', max(Col4) \'\'";0);1);keys;CHOOSECOLS(q;1);ids;ARRAYFORMULA(IF(LEFT(keys;3)="id:";MID(keys;4;999);""));catalogDisplay;ARRAYFORMULA(IFNA(VLOOKUP(ids;FILTER(HSTACK(ARRAYFORMULA(TRIM(\'Cartões\'!A2:A));\'Cartões\'!B2:B);ARRAYFORMULA(TRIM(\'Cartões\'!A2:A))<>"";\'Cartões\'!B2:B<>"");2;FALSE);""));rowDisplay;ARRAYFORMULA(IFNA(VLOOKUP(ids;FILTER(HSTACK(ARRAYFORMULA(TRIM(\'Lançamentos Cartão\'!G2:G));\'Lançamentos Cartão\'!H2:H);ARRAYFORMULA(TRIM(\'Lançamentos Cartão\'!G2:G))<>"";\'Lançamentos Cartão\'!H2:H<>"");2;FALSE);""));display;ARRAYFORMULA(IF(LEFT(keys;3)="id:";IF(catalogDisplay<>"";catalogDisplay;IF(rowDisplay<>"";rowDisplay;ids));REGEXREPLACE(keys;"^legacy:";"")));VSTACK(HSTACK("Cartão";"Mês de Cobrança";"Total da Fatura";"Parcelas Lançadas";"Primeira Compra";"Última Compra");HSTACK(display;CHOOSECOLS(q;2;3;4;5;6))));HSTACK("Cartão";"Mês de Cobrança";"Total da Fatura";"Parcelas Lançadas";"Primeira Compra";"Última Compra"))',
        '',
        '',
        '',
        '',
