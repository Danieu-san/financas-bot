'use strict';
// Mechanical extraction/check only: never runs the evaluator or applies graphs.
...
const base = '3fc0f3d83ac5e418a802ca93f097b0c7459fface';
const read = file => fs.readFileSync(path.join(root, file), 'utf8').replaceAll('\r\n', '\n');
const hash = bytes => `sha256:${createHash('sha256').update(bytes).digest('hex')}`;
const git = args => execFileSync('git', args, { cwd: root, encoding: 'utf8', maxBuffer: 16 * 1024 * 1024,
    stdio: ['ignore', 'pipe', 'pipe'] });
const filename = path.join(__dirname, 'compensation-source-validation.json');
...
function build(mode) {
    ...
    const saved = previous?.wide || JSON.parse(read('.codex-temp/wide-n02g-compensation-source-20260922.json'));
    ...
    assert.equal(saved.tap.pass, 2365); assert.equal(saved.tap.fail, 0); assert.equal(saved.tap.skipped, 10);
    ...
    const unchanged = ['docs/contracts/next/provenance-v2/graphs-v2.json', 'docs/contracts/next/provenance-v2/claims-v2.json',
        'src/next/provenance/metricSelection.js', 'scripts/agent/reportNextCausalAuthoring.cjs',
        ...
        'docs/contracts/next/provenance-v2/causal-authoring-candidates/consumption-by-instrument-profile-v1.json',
        'docs/contracts/next/provenance-v2/causal-authoring-candidates/statement-total-profile-v1.json'].map(file => {
    ...
    const affected = previous?.affected || JSON.parse(read('.codex-temp/n02g-compensation-source-affected.json'));
    assert.equal(affected.exit_status, 0); assert.equal(affected.tap.pass, 108); assert.equal(affected.tap.fail, 0);
    assert.equal(affected.tap.skipped, 0); assert.equal(affected.original_six_candidate_reports_unchanged, true);
    ...
    return { schema: 'n02g-compensation-source-validation-v1', status: 'candidate_awaiting_independent_audit',
        evidence_kind: 'locally_executed_not_external_verdict', base, graph_accepted: false, releaseEligible: false,
        normative_delta_applied: false, correction: 'reject compensation source class other than eligible_class before financial filtering',
        red_observed: { test: 'AUTHOR-GENERATE-011', fact_key: 'M-02#1#1', source_class: 'income', state: 'confirmed',
            failure: 'Missing expected exception: source must fail before filtering' },
        focal: { tests: 2, pass: 2, fail: 0, skipped: 0, generator_cases: 24, runtime_cases: 8 }, affected,
        ...
        note: 'Helper added after final wide run; no causal source/test changed afterwards. Check validates saved records/current bytes, not past execution. Synthetic mutants do not authenticate sources or host. No normative application.' };
}
