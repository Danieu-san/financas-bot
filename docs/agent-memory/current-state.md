...
- Etapa 4.5 acelerada iniciada em 2026-06-19: foi criado o gate `npm run gate:enforce:accelerated` para substituir espera passiva por evidencia ativa antes da auditoria final. Ele consolida bateria offline, shadow pos-cutoff, E2E real verificado, rollback por flag e logs limpos. O resultado verde e apenas `READY_FOR_ALTISSIMA_AUDIT`, nunca ativacao automatica. A bateria IRAB passou a ter 350 casos, incluindo confirmacao para `income.create` quando campo critico depende de inferencia. Relatorio: `docs/qa/accelerated-enforce-gate-2026-06-19.md`. Enquanto E2E/rollback/logs nao forem marcados como verificados, o status correto continua `KEEP_SHADOW`.
