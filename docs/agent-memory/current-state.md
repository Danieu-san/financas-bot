...
- 3F.1G tem `GO` local; 3F.1H continua `NO-GO` e exige canario read-only,
  cenarios de falha/restart/follow-up/custo e E2E manual pelo WhatsApp.
- Os commits `181485e` e `a7df554` corrigem rotas de escrita especificas e nao
  comprovam 3F.1H.
- A variacao aprovada libera o canario somente para os dois membros autorizados
  do casal; demais usuarios permanecem fora do produto/baseline.
- 3G e 3H continuam pendentes. A Fase 3 esta aberta e a Fase 4 nao foi iniciada.
- Valores e identificadores de contas reais foram redigidos destes registros
  publicos; contagens, resultados de testes e marcadores tecnicos permanecem.
...
- Manual WhatsApp E2E after deploy on 2026-07-09 passed the three read-only prompts: recent expenses on the member B card returned the expected four latest rows; weekday spending returned quinta-feira with `R$ [redigido]`; dashboard `Disponivel` explanation returned June 2026 value `R$ [redigido]` with saldo `R$ [redigido]`, reserva liquida `R$ [redigido]` and no internal ID/token leak.
...
- WhatsApp E2E on 2026-07-08 caught a public wording/routing gap: the first reimbursement receipt was logged as `intent=entrada`. Hotfix `3834bb0` keeps the legacy sheet write compatible but presents/logs compensation income as `Reembolso` / `intent=reimbursement`, skips ordinary-income reserve suggestions, and passed the remote focused regression. Retest marker `TESTE_APAGAR_3F_E2E_FIX_20260708` returned the correct WhatsApp confirmation.
- E2E cleanup after the hotfix removed both test markers from Google Sheets, canonical shadow and read-model SQLite. Cleanup backup retained at `/home/ubuntu/financas-bot/data/backups/canonical_ledger_shadow.pre-phase3f-e2e-cleanup-20260708T2215Z.sqlite`.
...
- Marker-only smoke `TESTE_APAGAR_PHASE3D_20260705_1310` proved: two payables listed at `R$ [redigido]` and `R$ [redigido]`; payable total `R$ [redigido]`; one receivable at `R$ [redigido]`; the relative next-15-days query also returned `R$ [redigido]`; pending items did not alter the current account balance.
...
- Remote focused tests passed `43/43`: `canonicalLedgerProjector`, `canonicalLedgerShadowStore`, `canonicalLedgerReceiptProjector` and `canonicalLedgerParityReport`.
- Pre-restart SQLite backup was created at `data/backups/canonical_ledger_shadow.pre-phase3b-20260705T003027Z.sqlite`. An earlier mistyped backup filename `data/backups/canonical_ledger_shadow.pre-phase3b-.sqlite` also exists and was left untouched rather than deleting backup data during deploy.
- PM2 restarted `financas-bot`; dashboard health returned `{"ok":true,"sqlite":true}`; logs showed WhatsApp authenticated and ready; remote `state_store.json` parsed as valid JSON.
...
- A deteccao de cartao pessoal citado na mensagem agora ignora diferencas de pontuacao e separadores, por exemplo, variacoes de pontuacao e separadores do mesmo alias sanitizado, sem escolher silenciosamente quando mais de um cartao combina com um nome generico.
...
- Correcao em 2026-05-31: quando o usuario cita um cartao cadastrado pelo nome (ex.: `cartao do membro B`) sem dizer `debito`, o bot trata como cartao de credito mesmo se a IA classificar equivocadamente como `Débito`; `à vista` vira parcela `1/1`. Se o usuario disser explicitamente `debito`, o fluxo de debito e preservado.
- Dado real corrigido em 2026-05-31: `estabelecimento redigido` de R$ [redigido] foi movido de `Saídas/Débito` para `Lançamentos cartao do membro B`, apos backup `Backup FinancasBot Daniel antes mover estabelecimento redigido 2026-05-31 1249`.
...
- Dado real corrigido em 2026-05-31: o lançamento `estabelecimento redigido` de R$ [redigido] do Daniel em `Lançamentos Cartão` foi ajustado de `31/05/2026` para `30/05/2026 22:00`, apos backup `Backup FinancasBot Daniel antes ajustar data estabelecimento redigido 2026-05-31T14-16-28-865Z`.
...
- Saldo economico: R$ [redigido].
- Aplicado em reserva: R$ [redigido].
- Resgatado da reserva: R$ [redigido].
- Reserva liquida: R$ [redigido].
- Disponivel estimado: R$ [redigido].
...
- Sintoma: `quanto gastei esse mês?` respondia R$ [redigido], mas follow-ups como `detalhe os gastos pra mim`, `foram em quais estabelecimentos?` e `e por categoria?` caiam para R$ [redigido] porque alguns caminhos usavam data da compra enquanto outros usavam mes da fatura.
...
- Sintoma: `quanto alimentação representa do total de gastos?` usava R$ [redigido]/R$ [redigido] em vez de R$ [redigido]/R$ [redigido], pois `percentual_categoria_gastos` ainda passava pelo calculo legado por data da compra.
...
- Nomes de cartao nao concedem identidade/permissao de membro. Consultas como fatura do `cartao do membro B` continuam pessoais salvo pedido familiar ou por pessoa explicitamente autorizado.
...
- Decisao operacional em 2026-07-02: gates do roadmap nao devem mais ficar em espera passiva para acumular uso natural. Toda exigencia de "esperar e observar" deve gerar uma bateria adversarial, detalhada, repetivel e limpavel que cubra caminhos felizes, ambiguidades, cancelamentos, falhas, replay/idempotencia, datas, reinicio, paridade, telemetria, rollback e limpeza. A Fase 2 ganhou um `Accelerated Evidence Gate` especifico para Command Planner e canonical ledger. Espera cronologica so permanece quando um risco externo for comprovadamente impossivel de simular; E2E dificil deve ser transferido para envio manual por Daniel, sem espera ativa.
- Gate acelerado do Command Planner/ledger concluido em 2026-07-02. Evidencia: planner offline `11/11`, Gemini live `11/11`, dry-run ledger com 15 eventos/zero diferencas/privacy OK, E2E real de salvar e cancelar `bill.pay`, `debt.pay`, `invoice.pay` e `expense.create`, telemetria principal com 16 eventos/zero erro/zero critico/p95 planner 5583 ms/p95 handler 1460 ms, suite final `640/640` e audit high zero. A rodada encontrou `created_at=1970` nos recibos reais do shadow; o commit `6377747` passou a usar o timestamp estavel do Financial Write Ledger. Smoke pos-fix comprovou `created_at=2026-07-02T12:07:33.500Z`, `bill_payment` e `free_budget_eligible=0`. Fixtures e ledger foram limpos com backups; producao terminou em `6377747`, PM2/WhatsApp/health verdes, state vazio e flags preservadas. Veredito: GO para seguir ao proximo corte planejado, sem liberar route global, enforce ou novos dominios canonicos.
- Guard local em 2026-07-02 no Step 8/canonical ledger: o dominio canary `accounts` foi mantido fail-closed mesmo quando incluido em `CANONICAL_LEDGER_CANARY_READ_DOMAINS`, retornando `canonical_accounts_opening_balances_unavailable`. Motivo: a projecao atual de recibos so permite calcular deltas de movimento por `account_name`, nao saldo real com `ledger_accounts` e `opening_balance_cents`; chamar isso de `balance_cents` poderia gerar respostas financeiras erradas. O caminho morto que agregava linhas como saldo foi removido. RED/GREEN em `tests/canonicalLedgerReceiptProjector.test.js`; focado `10/10`. Proximo corte canonico deve seguir para um dominio elegivel com gate proprio, ou implementar antes a projecao real de contas/saldos.
...
- Evidencia local: RED/GREEN em `tests/canonicalLedgerReceiptProjector.test.js`, `tests/userSpreadsheetService.test.js` e `tests/unit.test.js`; focados passaram `202/202`, `npm test` passou `646/646`, `npm audit --audit-level=high` sem vulnerabilidades, `git diff --check` sem erros, scan NUL sem ocorrencias, `state_store.json` `{}`.
...
- Atualizacao em 2026-07-03: Daniel informou os saldos reais para `Contas Financeiras` e a aba central foi saneada, removendo as 4 linhas ficticias `TESTE_APAGAR_ACCOUNTS_SOURCE_20260702`. Foram cadastradas 4 contas reais com data de corte `03/07/2026`: `conta corrente do membro A` (`bank`, R$ [redigido]), `conta-reserva do membro A` (`reserve`, R$ [redigido]), `conta corrente do membro B` (`bank`, R$ [redigido]) e `segunda conta do membro B` (`bank`, R$ [redigido]), vinculadas aos user_ids ativos de Daniel/Thais. Gate source-backed temporario no EC2 projetou essas linhas no SQLite shadow, leu `accounts` via canary com `GO`, privacidade OK e cleanup zero. Proximo passo: decidir ativacao controlada de `CANONICAL_LEDGER_CANARY_READ_DOMAINS=accounts` com backup de `.env`, SIGHUP/PM2 e smoke read-only; ainda nao ativado nesta etapa.
...
- Segunda fatia local em 2026-07-02/03: `messageHandler` passou a transformar `migrationGap` em log/metrica sanitizados (`[financial-agent] migration_gap ...`) sempre que o Financial Agent identifica lacuna e cai para o legado. A telemetria preserva somente tag/reason/tool/domain/action allowlisted/sanitizados, sem mensagem bruta, telefone, `user_id`, planilha, tokens ou raw rows. Regressao em `tests/financialAgent.test.js` cobre redacao de campos adversariais.
- Terceira fatia local em 2026-07-02/03: criada a bateria acelerada `scripts/runFinancialAgentMigrationGapBattery.js` (`npm run test:financial-agent:migration-gaps`) para substituir espera passiva na reducao do legado analitico. Ela executa 6 casos adversariais sinteticos sem Gemini, forca lacunas de planner, filtro/dominio invalido, escopo ausente e dimensao nao suportada, exige `migrationGap` sanitizado e falha por padrao se houver lacuna sem telemetria ou vazamento. Evidencia local: RED/GREEN em `tests/financialAgentMigrationGapRunner.test.js`, runner `6/6`, `gemini_calls=0`, `missing=0`, `unsafe=0`, tags observadas `engine_gap=2`, `unsupported_filter=3`, `ambiguous_scope=1`.
- Quarta fatia local em 2026-07-03: criado o gate consolidado `scripts/runAnalyticalLegacyReductionGate.js` (`npm run gate:analytical-legacy`) para juntar a bateria ampla do Financial Agent com a bateria de `migrationGap`. O gate produz veredito `GO/NO_GO`, falha por padrao se houver gap de acceptance, lacuna sem telemetria ou telemetria insegura, e grava somente sumarios/caminhos de relatorio. Evidencia local: RED/GREEN em `tests/analyticalLegacyReductionGate.test.js`; gate completo `GO`, acceptance `265/265`, migration gaps `6/6`, `gemini_calls=0`, blockers `none`.
- Ajuste corretivo da quarta fatia em 2026-07-03: o gate `gate:analytical-legacy` agora força `FINANCIAL_AGENT_LLM_PLANNER_ENABLED=false` e `FINANCIAL_CONTEXTUAL_ANALYST_MODE=off` durante as baterias offline, restaurando as flags originais ao final. Isso evita que a EC2, cujo baseline tem Gemini Planner e contextual analyst ativos, gaste chamadas Gemini ao rodar um gate que deve ser deterministico e `gemini_calls=0`. Regressao no teste do gate cobre ambiente com planner/contextual analyst previamente ligados e falha se o analista contextual tentar chamar LLM.
- Quinta fatia local em 2026-07-03: adicionada a trava controlada por dominio `FINANCIAL_AGENT_ANALYTICAL_LEGACY_FALLBACK_DISABLED_DOMAINS` para bloquear fallback analitico legado apenas quando `FINANCIAL_AGENT_MODE=answer` e o Financial Agent devolver `migrationGap` do dominio listado. Valor vazio preserva comportamento atual; `shadow` continua observacional mesmo com a flag configurada; `*` bloqueia todos os dominios com lacuna rastreada. Quando bloqueado, o bot responde com mensagem segura sem expor detalhes internos, registra metrica/log sanitizados e nao cai no calculo legado. Evidencia TDD em `tests/financialAgent.test.js`; ainda nao ativar a flag em producao antes de gate por dominio.

## Roadmap alignment routine - 2026-07-03

- Rotina permanente registrada em `docs/agent-memory/README.md`: antes de fatia nao trivial, conferir o roadmap geral, declarar fase/subplano, escopo, gate e proximo passo; durante a execucao, reconciliar qualquer correcao lateral com o roadmap; no fechamento, atualizar memoria com GO/NO-GO, flags e proximo passo. Remocao ampla de legado fica explicitamente ancorada na Fase 8.
- Fase atual do roadmap geral: Fase 2 - Contas, datas e status. Subfatia ativa: `accounts` do canonical ledger, apos `transactions/transfers`.
- Descoberta de seguranca da subfatia: o gate real de `Contas Financeiras` passou, mas foi temporario e limpo; portanto nao basta ligar `CANONICAL_LEDGER_CANARY_READ_DOMAINS=accounts`. Antes da ativacao, as contas reais precisam ser persistidas no SQLite shadow como baseline auditavel.
- Implementacao local criada: `scripts/runCanonicalLedgerAccountsSourceGate.js` e script npm `ledger:accounts-source-gate`. O runner aceita linhas exportadas da aba `Contas Financeiras`, exige `--confirm-real-source`, valida leitura canary `accounts`, escaneia privacidade e so deixa a projecao persistida quando `--persist-source` for usado explicitamente. Focado local `canonicalLedgerReceiptProjector` + `canonicalLedgerCanaryRouter` + `canonicalLedgerShadowStore` + `canonicalLedgerParityReport` passou `23/23`.
- Proximo passo do roadmap: concluir verificacao completa local, commitar/deployar o runner, executar o gate source-backed persistente no EC2 com as contas reais, e somente depois decidir a ativacao controlada de `accounts` nas flags de canary read.

## Canonical ledger accounts canary activation - 2026-07-03

- Commit/runtime `9f4f806` deployed on EC2. Focados remotos de ledger/accounts passaram `19/19`.
- Antes da ativacao, foi feito backup do SQLite shadow em `data/backups/canonical_ledger_shadow.pre-accounts-source-20260703T1220Z.sqlite` e backup do `.env` em `/home/ubuntu/financas-bot-backups/.env.pre-canonical-accounts-canary-20260703T1225Z`.
- A aba real `Contas Financeiras` foi exportada para JSON temporario no servidor com 4 contas esperadas. O runner `ledger:accounts-source-gate` persistiu `ACCOUNTS_SOURCE_REAL_20260703` no SQLite shadow com `decision=GO`, `privacy.ok=true`, `rows=4` e `persistentRun=true`.
...
- Smoke read-only direto do canary `accounts` retornou `enabled=true` e os saldos publicos: `conta corrente do membro A` R$ [redigido]; `conta-reserva do membro A` R$ [redigido]; `conta corrente do membro B` R$ [redigido]; `segunda conta do membro B` R$ [redigido]. PM2 online, health `{"ok":true,"sqlite":true}`, WhatsApp ready e `state_store.json` remoto `{}`.
...
- Causa raiz confirmada em producao: UserSettings tinha duas linhas para Daniel; a primeira continha SIM, R$ [redigido], escopo family e ciclo dia 28, enquanto a segunda estava vazia. O alerta lia a configuracao canonica por usuario, mas o read-model processava ambas e o upsert SQLite da linha vazia sobrescrevia a configuracao ativa.
...
- A matriz cobre gasto settled em 04/07, entrada settled em 05/07, transferencia settled e neutra em 06/07 e transferencia pending em 07/07. O saldo esperado/canonico ficou R$ [redigido] na origem e R$ [redigido] no destino; a pendencia nao alterou saldo.
...
## Canonical ledger explicit financial account contract deploy - 2026-07-03

- Commit `1a4c30f` deployado na EC2 por fast-forward, sem alterar `.env` nem flags. Focados remotos `tests/canonicalLedgerReceiptProjector.test.js` + `tests/userSpreadsheetService.test.js` passaram `29/29`.
...
- Escopo desta fatia: `expense.create` planejado pelo Gemini command planner, fluxo legado de gasto apos metodo de pagamento/categoria e fluxo de entrada apos metodo de recebimento. Credito, faturas, contas recorrentes completas, importacao em lote e remocao de legado ficam fora deste corte.
- A selecao aceita numero ou nome exato da conta, respeita o escopo financeiro familiar retornado por `getFinancialScopeUserIds`, ignora contas inativas/fechadas e grava a escolha na coluna final `Conta Financeira` sem deslocar as colunas antigas.
- Confirmacoes de gasto planejado agora mostram `Conta: ...` quando a conta foi escolhida; a chave idempotente de `expense.create` inclui a conta financeira para evitar replay incorreto entre contas diferentes.
- Evidencia TDD: tres testes RED/GREEN em `tests/financialStateMachine.test.js` cobrem gasto legado com PIX, gasto planejado via command planner com escopo familiar Daniel/Thais e entrada recebida por PIX.
...
- Smoke produtivo repetido apos reparar a fonte familiar: gasto PIX de R$ [redigido] pediu conta de origem, exibiu `conta corrente do membro A` na confirmacao e gravou a coluna `Conta Financeira`; entrada PIX de R$ [redigido] pediu conta de destino e gravou a mesma conta. O fluxo de credito ja havia passado sem pergunta de conta financeira.
- Sheets e ledger shadow foram conferidos antes da limpeza. Os dois eventos estavam `settled`, com direcoes `outflow`/`inflow` e linhas vinculadas a `conta corrente do membro A`.
...
- Saldos canonicos retornaram ao baseline: conta corrente do membro A R$ [redigido]; conta-reserva do membro A R$ [redigido]; conta corrente do membro B R$ [redigido]; segunda conta do membro B R$ [redigido]. Health e SQLite verdes; `state_store.json` valido.
...
- Smoke produtivo manual com `TESTE_APAGAR_TRANSFER_ACC_20260703`: o bot perguntou conta origem e destino, impediu destino igual a origem pela lista filtrada, confirmou `conta-reserva do membro A -> conta corrente do membro A`, data `04/07/2026`, status `Concluída` e gravou a transferencia.
- Smoke produtivo manual com `TESTE_APAGAR_TRANSFER_PEND_20260703`: a transferencia familiar pendente passou pela previa, preservou data `10/07/2026`, origem `conta corrente do membro A`, destino `conta corrente do membro B`, status `Pendente` e o cancelamento final nao gravou nada.
...
- Filtros por titular/conta preferem correspondencia exata: `Daniel Nubank` retorna R$ [redigido] sem somar a Caixinha; `Thais Itau` retorna R$ [redigido]. Consultas amplas como `caixinha` continuam validas.
...
- Manual smoke passed with exact values: Daniel total R$ [redigido]; caixinha R$ [redigido]; conta corrente do membro A R$ [redigido]; segunda conta do membro B R$ [redigido]; member B card R$ [redigido].
...
- Remote dashboard/read-model validation rebuilt from the production `.env` and returned 4 sanitized financial accounts, total balance R$ [redigido], names `conta corrente do membro A`, `conta-reserva do membro A`, `segunda conta do membro B`, `conta corrente do membro B`, and `leaks=false`.
...
- Manual production smoke used the real `cartao do membro B` invoice for R$ [redigido] via PIX. The bot required a paying financial account, accepted `conta corrente do membro A` and displayed that account in the final invoice-payment confirmation.
