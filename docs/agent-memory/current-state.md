...
- Ajuste corretivo da quarta fatia em 2026-07-03: o gate `gate:analytical-legacy` agora força `FINANCIAL_AGENT_LLM_PLANNER_ENABLED=false` durante as baterias offline e restaura a flag original ao final. Isso evita que a EC2, cujo baseline tem Gemini Planner ativo, gaste chamadas Gemini ao rodar um gate que deve ser deterministico e `gemini_calls=0`. Regressao no teste do gate cobre ambiente com planner previamente ligado.