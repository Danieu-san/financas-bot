...
- Atualizacao em 2026-07-03: Daniel informou os saldos reais para `Contas Financeiras` e a aba central foi saneada, removendo as 4 linhas ficticias `TESTE_APAGAR_ACCOUNTS_SOURCE_20260702`. Foram cadastradas 4 contas reais com data de corte `03/07/2026`: `Daniel - Nubank` (`bank`, R$ 262,85), `Daniel - Nubank Caixinha` (`reserve`, R$ 1.264,91), `Thais - Nubank` (`bank`, R$ 0,00) e `Thais - Itaú` (`bank`, R$ 133,46), vinculadas aos user_ids ativos de Daniel/Thais. Gate source-backed temporario no EC2 projetou essas linhas no SQLite shadow, leu `accounts` via canary com `GO`, privacidade OK e cleanup zero. Proximo passo: decidir ativacao controlada de `CANONICAL_LEDGER_CANARY_READ_DOMAINS=accounts` com backup de `.env`, SIGHUP/PM2 e smoke read-only; ainda nao ativado nesta etapa.
