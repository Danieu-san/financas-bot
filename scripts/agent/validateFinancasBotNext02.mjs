...
const base = '5d4339f46a9ec412d6c86894853435c7238dbcf1';
const allowed = [
...
if (!worktree) {
    ...
    errors.push(...prior.validateGitBindingEvidence({
        ...
        requiredFiles: [...allowed, ...policy.EXPECTED_PATHS.map(p => 'src/next/' + p)],
        trackedFiles: new Set(git('ls-tree', '-r', '--name-only', 'HEAD').split('\n')),
        ignoredPaths: git('ls-files', '--others', '--ignored', '--exclude-standard', '--', 'src/next', 'tests/next02ObservationKernel.test.js')
            .split('\n').filter(Boolean)
    }));
}
...
const analysis = policy.inspectSources(path.join(root, 'src/next'));
errors.push(...analysis.errors);
if (!errors.length) {
    const events = [];
    const stream = run({
        files: [path.join(root, 'tests/next02ObservationKernel.test.js')], concurrency: 1,
        setup(testStream) {
    ...
    const proof = policy.validatePropertyEvents(events);
    errors.push(...proof.errors);
    if (errors.length) process.stderr.write(diagnostic);
    else console.log('property_ids=' + proof.approvedIds.length + '/' + policy.REQUIRED_IDS.length);
}
if (errors.length) {
    console.error('NEXT-02 N02-A: FAIL\n' + errors.join('\n'));
    process.exitCode = 1;
} else {
    console.log(worktree ? 'NEXT-02 N02-A PRECOMMIT: PASS' : 'NEXT-02 N02-A GATE: PASS');
    console.log('source_tree_entries=' + policy.EXPECTED_PATHS.length);
    ...
    console.log('scope=N02-A_only; NEXT-02_full_gate=pending');
