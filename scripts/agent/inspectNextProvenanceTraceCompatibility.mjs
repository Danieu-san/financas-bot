...
// admitted graph package. Edge obligations and read obligations are exact,
// independent dimensions; this reports edge-only cases without calling them
// coverage gaps or fabricating a trace.
export function inspectTraversalCoverage(graphs) {
    ...
    for (const graph of graphs) for (const phase of ['derivation', 'proof']) {
        ...
        for (const edgeId of contract.required_edges) {
            ...
            if (missingNodes.length || missingReads.length) edgeOnly.push({ fact_key: graph.fact_key, phase,
    ...
    return { stage: 'independent_edge_read_compatibility_diagnostic', compatible: true,
        graphs_checked: graphs.length, phases_checked: graphs.length * 2,
        edge_only_graphs: new Set(edgeOnly.map(g => g.fact_key)).size,
        edge_only_count: edgeOnly.length, edge_only: edgeOnly };
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
    ...
    const { edge_only: edgeOnly, ...summary } = inspectTraversalCoverage(graphs);
    console.log(JSON.stringify({ ...summary, examples: edgeOnly.slice(0, 3) }, null, 2));
