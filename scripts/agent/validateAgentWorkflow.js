...
const checks = [
    ...
    ['docs/agent-memory/workstreams/index.md', 8 * 1024, ['wgl-03-wgl-04', 'aws-oracle-migration', 'codex-usage-calibration', 'chat-codex-orchestration', 'chat-codex-channel']],
    ...
    ['scripts/agent/chatAuditNotifier.js', 10 * 1024, ['AUDITORIA_FINANCASBOT_PRONTA', 'notification_id=', 'chat-notifier-sha256', 'already_sent']],
    ['scripts/agent/chatCodexAppWake.js', 10 * 1024, ['financasbot-codex-app-wake-request-v3', 'CODEX_READY', "const mode = 'execute'", 'app-wake-request']],
    ['scripts/agent/chatCodexTaskContract.js', 16 * 1024, ['financasbot-chat-codex-task-v1', 'allowed_paths', 'result_file']],
    ['scripts/agent/Install-ChatCodexOrchestrationWatcher.ps1', 12 * 1024, ['FinancasBot-ChatCodex-Orchestration', 'IgnoreNew', 'Interactive', 'RunNow', 'chat-notifier-script', 'chat-notifier-sha256']],
    ...
    ['tests/chatAuditNotifier.test.js', 10 * 1024, ['GitHub imutável', 'falha de envio pode ser tentada novamente', 'não usa a ponte direta do Codex App']],
    ...
    ['.agents/skills/audit-immutable-gate/SKILL.md', 14 * 1024, ['name: audit-immutable-gate', '## Fontes independentes', '## Auditoria pelo Chat', 'chat-codex-channel.state.json', 'GitHub continua sendo a autoridade']],
